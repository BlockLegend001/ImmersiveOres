...
public class EnderiumExcavator extends Excavator {
    private static Properties createSettings(boolean unbreakable, int durability) {
        Properties settings = new Properties()
                ...
                .attributes(EnderiumExcavator.createAttributes(
                        ModToolTiers.ENDERIUM,
                        EnderiumConfig.ATTACK_DAMAGE_ENDERIUM_EXCAVATOR.get(),
                        (float) EnderiumConfig.ATTACK_SPEED_ENDERIUM_EXCAVATOR.get().doubleValue()
                ));
    ...
    public EnderiumExcavator(Tier pTier, int pAttackDamageModifier, float pAttackSpeedModifier, Properties pProperties) {
        super(pTier, pAttackDamageModifier, pAttackSpeedModifier, createSettings(EnderiumConfig.UNBREAKABLE_ENDERIUM.get(), EnderiumConfig.DURABILITY_ENDERIUM.get()));
    }
