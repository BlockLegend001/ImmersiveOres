...
public class EnderiumBow extends BowItem {
    ...
    private final int ARROW_COUNT = EnderiumConfig.ARROW_COUNT_ENDERIUM_BOW.get();
    ...
    public EnderiumBow(BowTier tier, Properties properties) {
        super(createSettings(properties, EnderiumConfig.UNBREAKABLE_ENDERIUM.get(), EnderiumConfig.DURABILITY_ENDERIUM.get()));
        this.tier = tier;
    }
    ...
    @Override
    public void appendHoverText(ItemStack pStack, TooltipContext pContext, TooltipDisplay display, Consumer<Component> components, TooltipFlag pTooltipFlag) {
        int arrowCount = getArrowCount(pStack);
        ChatFormatting color = ChatFormatting.DARK_AQUA;
        ...
        if (ScreenUtils.isShiftDown()) {
            ...
            components.accept(arrow);
        } else {
            Component pressShift = Component.translatable("tooltip.immersiveores.pressshiftformoreinfo.tooltip")
                    .withStyle(color);
            components.accept(pressShift);
        }
    }

    private int getArrowCount(ItemStack stack) {
        if (ArrowCountMap.getEnderiumBowArrowCount().containsKey(stack.getItem())) {
            return ArrowCountMap.getEnderiumBowArrowCount().get(stack.getItem());
        }
        return 0;
    }
}