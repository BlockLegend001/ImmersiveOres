...
public class VulpusBow extends BowItem {
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        if (user instanceof PlayerEntity player) {
            ...
            Enchantment enchantmentReference = enchantmentRegistry.getEntry(Enchantments.INFINITY).value();
            boolean hasInfinity = EnchantmentHelper.getLevel(enchantmentReference, player.getMainHandStack()) > 0;

            Enchantment enchantmentReferencePower = enchantmentRegistry.getEntry(Enchantments.POWER).value();
            int powerLevel = EnchantmentHelper.getLevel(enchantmentReferencePower, player.getMainHandStack());

            Enchantment enchantmentReferencePunch = enchantmentRegistry.getEntry(Enchantments.PUNCH).value();
