...
public class ModEventsVibraniumExcavator {
    ...
    public static void onExcavatorUsage(BlockEvent.BreakEvent event) {
        ...
        if(mainHandItem.getItem() instanceof VibraniumExcavator excavator && player instanceof ServerPlayer serverPlayer) {
            BlockPos initalBlockPos = event.getPos();
            if (HARVESTED_BLOCKS.contains(initalBlockPos)) {
                return;
            }

            for (BlockPos pos : VibraniumExcavator.getBlocksToBeDestroyed(1, initalBlockPos, serverPlayer)) {
